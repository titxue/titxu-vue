<template>
  <div>
    ...
    <el-table
      ...
      :default-expand-all="xTable.table.expandAll"
      ...
      @row-dblclick="xTable.onTableDblclick"
      v-bind="$attrs"
    >
      <slot name="columnPrepend"></slot>
      <template v-for="(item, key) in xTable.table.column">
    ...
    <div v-if="props.pagination" class="table-pagination">
      <el-pagination
        :currentPage="xTable.table.filter!.page"
        :page-size="xTable.table.filter!.limit"
        :page-sizes="pageSizes"
        background
        :layout="config.layout.shrink ? 'prev, next, jumper' : 'sizes,total, ->, prev, pager, next, jumper'"
        :total="xTable.table.total"
...
<script setup lang="ts">
  ...
  import Column from '/@/components/v1/table/column/index.vue';
  import FieldRender from '/@/components/v1/table/fieldRender/index.vue';
  ...
  const onTableSizeChange = (val: number) => {
    xTable.onTableAction('page-size-change', { size: val });
  };

  const onTableCurrentChange = (val: number) => {
    xTable.onTableAction('current-page-change', { page: val });
  };

  const onSortChange = ({ order, prop }: { order: string; prop: string }) => {
    xTable.onTableAction('sort-change', { prop: prop, order: order ? (order == 'ascending' ? 'asc' : 'desc') : '' });
  };

  const pageSizes = computed(() => {
    let defaultSizes = [10, 20, 50, 100];
    if (xTable.table.filter!.limit) {
      if (!defaultSizes.includes(xTable.table.filter!.limit)) {
        defaultSizes.push(xTable.table.filter!.limit);
      }
    }
    return defaultSizes;
  });
  ...
  const onSelectAll = (selection: TableRow[]) => {
    if (isSelectAll(selection.map((row: TableRow) => row[xTable.table.pk!].toString()))) {
  ...
  const isSelectAll = (selectIds: string[]) => {
    let data = xTable.table.data as TableRow[];
    for (const key in data) {
      return selectIds.includes(data[key][xTable.table.pk!].toString());
    }
    return false;
  };
  ...
  const onSelect = (selection: TableRow[], row: TableRow) => {
    if (
      selection.some((item: TableRow) => {
        return row[xTable.table.pk!] === item[xTable.table.pk!];
      })
    ) {
      if (row.children) {
        selectChildren(row.children, true);
  ...
  const onSelectionChange = (selection: TableRow[]) => {
    xTable.onTableAction('selection-change', selection);
  };
  ...
  const unFoldAll = (unfold: boolean) => {
    setUnFoldAll(xTable.table.data!, unfold);
  };
