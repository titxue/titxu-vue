<template>
  ...
  <div v-if="field.render === 'image' && fieldValue" class="ba-render-image">
    <el-image :hide-on-click-modal="true" :preview-teleported="true" :preview-src-list="[fullUrl(fieldValue)]" :src="fullUrl(fieldValue)" />
  </div>
    ...
    <template v-for="(btn, idx) in field.buttons" :key="idx">
      <template v-if="btn.display ? btn.display(row, field) : true">
        ...
        >
          <template #reference>
            <div class="ml-6">
              <el-tooltip :disabled="!btn.title" :content="btn.title ? btn.title : ''" placement="top">
                <el-button
...
<script setup lang="ts">
  ...
  import { timeFormat, openUrl } from '/@/components/v1/table';
  ...
  const onChangeField = (value: any) => {
    xTable.onTableAction('field-change', { value: value, ...props });
  };
  ...
  const onButtonClick = (btn: OptButton) => {
    ...
    xTable.onTableAction(btn.name, props);
  };
