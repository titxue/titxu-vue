<template>
  <el-dialog v-model="dialogVisible" :fullscreen="fullscreen || false" :title="title" :width="width || '50%'"
    :draggable="draggable || true" v-bind="_options">
    <el-form class="px-15 pb-10" @submit.prevent :model="form" ref="formRef" label-position="right" label-width="80px"
      style="max-width: 100%">
      <template v-for="(item, index) in fieldList" :key="index">
        <!-- 单选框 -->
        <el-form-item :label="item.label" v-if="item.type === 'radio' && index !== undefined" :rules="item.rules"
          :prop="[item.field]">
          <el-radio-group v-model="form[item.field]" :disabled="item.disabled">
            <el-radio :label="val[item.options?.valueKey || 'value']"  size="default" v-for="val in item.options?.data"
              :key="val[item.options?.valueKey || 'value']" border>
        ...
        <el-form-item :label="item.label" v-else-if="item.type === 'select'" :rules="item.rules" :prop="[item.field]">
          ...
          <el-select v-model="form[item.field]" :placeholder="item.options?.placeholder || '请选择'"
        ...
        <el-form-item :label="item.label" :rules="item.rules" :prop="[item.field]" v-else>
          <el-input v-model="form[item.field]" :readonly="item.readonly" :type="item.type ?? 'text'"
            :placeholder="item.placeholder || item.label" :disabled="item.disabled" :showPassword="item.showPassword"
            :clearable="item.clearable" @keyup.enter="handleKeyUp(item.enterable)" />
        </el-form-item>
      </template>
