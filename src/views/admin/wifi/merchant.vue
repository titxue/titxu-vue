...
<script setup lang="ts">
  ...
  const formRef = ref();
  const tableRef = ref();
  const xTable: xTableClass<MerchantType> = new xTableClass(
    new xTableApi(ADMIN_URL.merchants),
    {
      dblClickNotEditColumn: [undefined],
      column: [
        { type: 'selection', align: 'center', operator: false },
        { label: '用户昵称', prop: 'name', align: 'center', operator: 'LIKE' },
        { label: '邮箱', prop: 'email', align: 'center', width: '170' },
        { label: '姓名', prop: 'contactName', align: 'center', operator: 'LIKE' },
        { label: '电话', prop: 'contactPhone', align: 'center', operator: 'LIKE' },
        {
          label: '状态',
          prop: 'status',
          align: 'center',
